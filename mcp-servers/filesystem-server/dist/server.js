...
server.resource("fsList", new ResourceTemplate("fs://list/{+directoryPath}", { list: undefined }), async (uri, { directoryPath }) => {
    ...
    try {
        ...
        const content = entries.map(entry => `${entry.isDirectory() ? '[D]' : '[F]'} ${entry.name}`).join('\n');
        return {
...
server.resource("fsRead", new ResourceTemplate("fs://read/{+filePath}", { list: undefined }), async (uri, { filePath }) => {
...
// writeFile Tool
server.tool("writeFile", z.object({
...
server.tool("createDirectory", z.object({
