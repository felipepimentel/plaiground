...
interface ActiveMcpConnection {
    ...
    configName: string; // Name from the config file
    client: Client;
    transport: StdioClientTransport; // TODO: Union type with WebSocket transport later
    websocket: WebSocket; // The UI websocket connection
    status: 'connecting' | 'connected' | 'disconnected' | 'error';
    serverInfo?: SimpleServerInfo;
}

// Use a Map for easier lookup by server name
let configuredServers = new Map<string, McpServerDefinition>();
let autoDiscoveredServers = new Map<string, McpServerDefinition>();

// Rate limiting for list requests
const listRequestRateLimits = new Map<string, number>();
...
// Add category configuration interface
        ...
        if (fs.existsSync(CONFIG_FILE_PATH)) {
            ...
            try {
                ...
                if (parsedConfig && typeof parsedConfig.mcpServers === 'object') {
                    ...
                    const tempMap = new Map<string, McpServerDefinition>();
                    let count = 0;
                ...
                if (parsedConfig.autoDiscovery && typeof parsedConfig.autoDiscovery === 'object') {
                    const autoDiscoveryConfig = parsedConfig.autoDiscovery as AutoDiscoveryConfig;
                ...
                if (parsedConfig.connection && typeof parsedConfig.connection === 'object') {
                    connectionConfig = parsedConfig.connection as ConnectionConfig;
                ...
                if (parsedConfig.categories && typeof parsedConfig.categories === 'object') {
                    categoryConfig = parsedConfig.categories as CategoryConfig;
 ...
 */
function getAllServers(): Map<string, McpServerDefinition> {
 ...
 */
function getConnectedWebSockets(): Set<WebSocket> {
 ...
 */
function disconnectServerConnections(serverName: string) {
                    ...
                    if (data.jsonrpc === '2.0' && data.method) {
                        ...
                        if (data.method === 'tools/list') {
                            ...
                            const connectionId = data.params?.connectionId ||
                                Array.from(activeConnections.values()).find(
                                    conn => conn.websocket === ws && conn.status === 'connected'
                                )?.id;
...
function sendConfiguredServersList(ws: WebSocket) {
...
async function handleConnectConfigured(ws: WebSocket, serverName: string) {
    const allServers = getAllServers();
    ...
    const existingConnection = Array.from(activeConnections.values()).find(
        conn => conn.configName === serverName && conn.status !== 'disconnected' && conn.status !== 'error'
    );
...
async function handleConnectStdio(ws: WebSocket, command: string, args: string[], configName: string, configCwd?: string) {
    ...
    let client: Client;
    let connection: ActiveMcpConnection | null = null;
    ...
    try {
        ...
        setTimeout(async () => {
            try {
                ...
                const serverInfo: SimpleServerInfo = {
                    name: configName,
                    version: "unknown",
                ...
                if (connection && connection.status === 'connecting') {
                    ...
                    const missingCapabilities = expectedCapabilities.filter(cap =>
                        !serverInfo.capabilities.includes(cap)
                    );
                    ...
                    if (serverCapabilities.includes('tools')) {
                        ...
                        sendWsMessage(ws, 'toolsList', {
                            connectionId,
                            data: { tools: toolsResult.tools as SimpleToolDefinition[] }
                        });
                    }
                    ...
                    if (serverCapabilities.includes('resources')) {
                        ...
                        sendWsMessage(ws, 'resourcesList', {
                            connectionId,
                            data: { resources: resourcesResult.resources as SimpleResource[] }
                        });
                    }
                    ...
                    if (serverCapabilities.includes('prompts')) {
                        ...
                        sendWsMessage(ws, 'promptsList', {
                            connectionId,
                            data: { prompts: promptsResult.prompts as SimplePromptDefinition[] }
                        });
                    }
                }
            } catch (err) {
...
function sendWsError(ws: WebSocket, errorMessage: string) {
    sendWsMessage(ws, 'mcpError', { error: errorMessage });
}

function markConnectionAsConnected(connection: ActiveMcpConnection, serverInfo: SimpleServerInfo) {
...
async function handleCallTool(ws: WebSocket, connectionId: string, toolName: string, args: any) { // args might be null/undefined
    const connection = activeConnections.get(connectionId);
    if (!connection || connection.status !== 'connected') {
    ...
    try {
        // Corrected call: Pass arguments as an object
        const result = await connection.client.callTool({ name: toolName, arguments: args as Record<string, unknown> | undefined });
...
async function handleGetPrompt(ws: WebSocket, connectionId: string, promptName: string, args: any) { // args might be null/undefined
    const connection = activeConnections.get(connectionId);
    if (!connection || connection.status !== 'connected') {
    ...
    try {
        // Corrected call: Pass arguments as an object
        const result = await connection.client.getPrompt({ name: promptName, arguments: args as Record<string, string> | undefined });
 ...
 */
async function autoConnectServers(websocket: WebSocket) {
 ...
 */
async function handleAutoReconnect(websocket: WebSocket, serverName: string) {
    ...
    const existingConnection = Array.from(activeConnections.values()).find(
        conn => conn.configName === serverName && (conn.status === 'connected' || conn.status === 'connecting')
    );
